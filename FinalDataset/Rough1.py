...
class rough1:
    def runRough1(self):
        data=pd.read_csv("SparseMatrix.csv")

        x_columns=list(data.columns)[:int(1909/2)]
        y_columns=list(data.columns)[int(1909/2):1909]
        ...
        from DayFive.StepSix import StringToInt
        ...
        #
        # with open('Found.pkl','wb') as file:
        #     pkl.dump(found,file)
        # file.close()